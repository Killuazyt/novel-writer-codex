...
The returned `request_file` is the only trusted request package for this run. Preserve the returned `binding_marker` exactly, and use the returned `agent_task_name` unchanged as the native child Agent's exact `task_name`. Invoke `webnovel_reviewer` as a depth-one native child Agent inside the current task. Do not shorten, decorate, reuse, or independently invent that task name. Do not create a Codex top-level task, use `create_thread`, or substitute the parent model. Give the child both the request-file path and the exact binding marker verbatim, and require one bare JSON object as its final answer. The reviewer is read-only and must not write files.
...
The runtime independently recomputes the task name from the immutable run/request binding marker; it never trusts a task name or marker self-reported by the accept request. It parses the explicit Codex rollout and requires depth `1`, exact `agent_path=/root/<agent_task_name>`, `webnovel_reviewer`, `gpt-5.6-luna`, and `medium`. Current Desktop rollouts may omit the marker's plaintext, so the verified Agent path is the binding boundary; only assistant `final`/`final_answer` records are eligible and commentary is ignored. An explicit-marker legacy trace remains supported. The runtime rejects reuse of a child or rollout across runs, binds the accepted raw output hash to the immutable `request.json`, and saves the task-name/path binding in the runtime receipt for recovery-time revalidation. It then verifies contract hash, chapter/context hashes, protected-state snapshots, exact fields, five dimensions, counts, and types. Agent self-report, a hand-written model label, caller-supplied response JSON, canned fixture, copied rollout, missing rollout, or parent-model fallback is not evidence. If the host cannot expose the explicit trusted rollout path and task identities, stop with the live gate pending; do not claim the review passed. Delete the temporary accept request with the host file API after use.
