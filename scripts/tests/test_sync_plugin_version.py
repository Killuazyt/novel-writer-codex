...
def _write_root(root: Path, *, version: str = "1.2.3", readme_marker: bool = True) -> None:
    _write_json(
...
def test_sync_versions_updates_manifest_and_existing_readme_marker(tmp_path):
    ...
    assert sync.find_readme_version((tmp_path / "README.md").read_text(encoding="utf-8")) == "1.2.4"
