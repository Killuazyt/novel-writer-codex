...
def _load_dotenv(project_root: Optional[Path] = None) -> None:
    ...
    优先级：进程环境 > 项目 `.env` > WEBNOVEL_HOME `.env` > 旧 Claude
    `.env`。旧路径只读；本函数不会在任何来源目录中创建或修改文件。

    本模块需要兼容连续打开不同项目，因此会撤销自己上一次注入且仍未
    被调用方修改的值，再按本次来源重新解析。外部显式设置的环境变量
    始终保留。
    """
    for key, managed_value in tuple(_DOTENV_MANAGED_VALUES.items()):
        if os.environ.get(key) == managed_value:
            os.environ.pop(key, None)
    _DOTENV_MANAGED_VALUES.clear()

    project_dir = Path(project_root) if project_root is not None else Path.cwd()
    sources = (
        project_dir / ".env",
        resolve_webnovel_home() / ".env",
        _get_user_claude_root() / "webnovel-writer" / ".env",
    )
    for source in sources:
        for key, value in _read_dotenv_file(source).items():
...
def _load_project_dotenv(project_root: Path) -> None:
    ...
    """
    try:
        _load_dotenv(Path(project_root))
    except Exception:
        return
