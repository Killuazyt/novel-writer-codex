...
成功时只返回五段完整任务书。输出必须从首行开始，标题逐字采用下列五个 Markdown H2，并严格保持顺序；不得添加前言、代码围栏、编号、额外 H2、检查清单、文件路径、合同字段名、工具日志或 run ledger：
...
## 这章的故事
[必要前情、本章目标与阻力、CBN/CPN/CEN 节点、必须覆盖项、禁区和跨章承接]

## 这章的人物
[每个出场人物的当前状态、驱动力、本章作用、知识边界与说话倾向]

## 怎么写更顺
[把节奏、题材、情绪、写法建议和项目文风翻译成具体指导，不暴露内部术语]

## 收在哪里
[结尾情绪、未完感和需要保留的钩子]
```

上面的代码围栏仅用于本合同展示 wire format；实际成功输出不得带围栏或方括号占位符。五段必须全部非空，并能让 writer 在不读取额外 canon 的情况下完成起草。不得在成功结果之外追加 `SubagentRun`、模型声明或大段来源内容。
